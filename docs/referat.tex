...
\Large Aplicatie de prognoza meteo probabilistica cu pipeline ML in PostgreSQL\\[0.5cm]
...
\textit{Coordonator: [Numele profesorului coordonator]}
\end{center}
\newpage
...
\begin{enumerate}
    ...
    \item \textbf{Stratul de date} --- PostgreSQL cu 21 de tabele, 39 de proceduri stocate, 6 functii, 3 triggeri si 20+ indecsi specializati pentru pipeline-ul ML.
\end{enumerate}
...
\centering
\textbf{Flux de date simplificat}\\[0.3cm]
Open-Meteo API $\rightarrow$ \texttt{WeatherImporterService} (Java) \\
$\rightarrow$ INSERT/UPDATE in \texttt{forecasts} (PostgreSQL) \\
$\rightarrow$ Trigger \texttt{trg\_forecasts\_updated\_at} \\
$\rightarrow$ Proceduri ML: \texttt{sp\_build\_weather\_vector} $\rightarrow$ \texttt{sp\_compute\_recipe\_scores} \\
$\rightarrow$ \texttt{sp\_run\_monte\_carlo} (predictii P10/P50/P90) \\
$\rightarrow$ JavaFX citeste rezultatele finale prin JDBC
}}
\caption{Flux end-to-end al datelor in aplicatie}
\end{figure}
...
La detectarea unui eveniment extrem, sistemul notifica automat toti utilizatorii care au interactionat recent cu prognozele din zona afectata.
\end{infobox}
...
\subsection{Extensia pentru pipeline ML}

Pipeline-ul de machine learning introduce tabele suplimentare care, prin design, sunt deja normalizate:
...
\toprule
\textbf{Tabela} & \textbf{PK} & \textbf{FK} & \textbf{1NF} & \textbf{2NF} & \textbf{3NF/BCNF} \\
\midrule
...
\texttt{comments} & id & $\rightarrow$ users, forecasts, comments & Da & Da & Da \\
...
\texttt{monte\_carlo\_predictions} & id & $\rightarrow$ cities & Da & Da & Da \\
...
\bottomrule
\end{tabular}
\caption{Verificarea formelor normale pentru toate tabelele}
\end{table}
...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Entitate sursa} & \textbf{Entitate tinta} & \textbf{Tip} & \textbf{Multiplicitate} \\
\midrule
\texttt{countries} & \texttt{cities} & compozitie & $1 : 0..*$ \\
\texttt{cities} & \texttt{forecasts} & compozitie & $1 : 0..*$ \\
\texttt{cities} & \texttt{hourly\_forecasts} & compozitie & $1 : 0..*$ \\
\texttt{cities} & \texttt{weather\_vectors} & compozitie & $1 : 0..*$ \\
\texttt{cities} & \texttt{daily\_regimes} & compozitie & $1 : 0..*$ \\
\texttt{cities} & \texttt{hidden\_states} & compozitie & $1 : 0..*$ \\
\texttt{cities} & \texttt{monte\_carlo\_predictions} & compozitie & $1 : 0..*$ \\
...
\texttt{comments} & \texttt{comments} & auto-asociere & $1 : 0..*$ \\
\bottomrule
\end{tabular}
\caption{Asocieri intre entitati}
\end{table}
...
\vspace{0.5cm}
\textbf{[Inserati aici diagrama generata din fisierul} \texttt{docs/er\_diagram.puml}\textbf{]}\\
\textit{Folositi PlantUML sau un editor online pentru a converti fisierul .puml in imagine .png}\\[0.3cm]
\texttt{plantuml docs/er\_diagram.puml}\\
\textit{sau incarcati continutul pe }\url{www.plantuml.com/plantuml}\textit{)}
\vspace{0.5cm}
}}
\caption{Placeholder pentru diagrama E/R UML}
\label{fig:er}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Prognoza pe zile}: un scroll orizontal cu carduri pentru urmatoarele 5 zile.
    \item \textbf{Detalii}: grid cu umiditate, viteza vantului, indice UV, presiune atmosferica, ore de soare, punct de roua.
    \item \textbf{Evolutie temperatura}: grafic LineChart cu selectare perioada (7 zile / 30 zile / 1 an).
    ...
    \item \textbf{Probabilitati meteo}: carduri cu probabilitati pentru precipitatii, canicula, furtuna.
...
\fbox{\parbox{0.85\textwidth}{\centering
\vspace{2cm}
\textbf{[Inserati aici screenshot-ul tab-ului ``Acum"]}\\[0.3cm]
\textit{Capturati ecranul aplicatiei ruland }\texttt{mvn javafx:run}\textit{, selectand un oras}\\[0.3cm]
\textbf{Nume fisier sugerat:} \texttt{screenshots/tab\_acum.png}
\vspace{2cm}
}}
\caption{Tab-ul ``Acum" --- Dashboard unificat}
\label{fig:tab_acum}
\end{figure}

\subsection{Tab-ul ``Harta"}

O harta interactiva a Europei desenata pe canvas JavaFX 2.5D. Fiecare tara este reprezentata ca poligon colorat in functie de temperatura medie. Orasele majore afiseaza badge-uri rotunde cu temperatura minima (stanga, albastru) si maxima (dreapta, rosu).

\begin{figure}[H]
\centering
\fbox{\parbox{0.85\textwidth}{\centering
\vspace{2cm}
\textbf{[Inserati aici screenshot-ul tab-ului ``Harta"]}\\[0.3cm]
\textbf{Nume fisier sugerat:} \texttt{screenshots/tab\_harta.png}
\vspace{2cm}
}}
\caption{Tab-ul ``Harta" --- Vizualizare Europei}
\label{fig:tab_harta}
\end{figure}

\subsection{Tab-ul ``Mai multe"}

Contine setari (unitati de temperatura, unitati de vant, sincronizare automata), sectiunea de comunitate (comentarii si voturi) si informatii despre aplicatie.

\begin{figure}[H]
\centering
\fbox{\parbox{0.85\textwidth}{\centering
\vspace{2cm}
\textbf{[Inserati aici screenshot-ul dialogului de comentarii]}\\[0.3cm]
\textbf{Nume fisier sugerat:} \texttt{screenshots/dialog\_comentarii.png}
\vspace{2cm}
}}
\caption{Dialogul de comentarii si voturi}
\label{fig:dialog_comentarii}
\end{figure}
...
\subsection{Pipeline ML --- Prezentare generala}

Pipeline-ul de machine learning ruleaza in 7 etape, toate implementate ca proceduri stocate:

\begin{algobox}[Pipeline ML end-to-end]
\begin{enumerate}
...
\fbox{\parbox{0.85\textwidth}{\centering
\vspace{1.5cm}
\textbf{[Inserati aici screenshot cu graficul predictiilor probabilistice]}\\[0.3cm]
\textbf{Nume fisier sugerat:} \texttt{screenshots/predictii\_probabilistice.png}
\vspace{1.5cm}
}}
...
\texttt{MlPipelineIntegrationTest} & Pipeline end-to-end & 1 & Pass \\
\midrule
...
Testul de integrare \texttt{MlPipelineIntegrationTest} valideaza intregul pipeline ML cu date reale de la Open-Meteo pentru Bucuresti: importa 365 de zile istorice, construieste vectori, ruleaza clustering, Markov, HMM si Monte Carlo, apoi verifica consistenta percentilelor ($\text{P10} \leq \text{P50} \leq \text{P90}$) si faptul ca toate probabilitatile sunt in $[0, 1]$.

% ========== 9. CONCLUZII ==========
\section{Concluzii}

Prezentul proiect demonstreaza ca un sistem de gestiune a bazelor de date relationale (PostgreSQL) poate gazdui nu doar operatii CRUD, ci si algoritmi sofisticati de machine learning si simulare statistica. Prin transferarea pipeline-ului de predictie la nivelul serverului, am obtinut:
